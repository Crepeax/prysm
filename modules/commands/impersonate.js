...
module.exports = {
    ...
    execute(message, args) {
        ...
        function sendHook(webhook) {
            let nick;
            if (client.guilds.get(message.guild.id).members.get(target.id).nickname == null) nick = target.username; else nick = client.guilds.get(message.guild.id).members.get(target.id).nickname;
            webhook.send(msg, {
                username: nick,
        ...
        if (hooks.first() == undefined) {
            message.channel.createWebhook(`${client.user.username}`, client.user.avatarURL)
            .then(hook => {
                sendHook(hook);
            });

    ...
    execute(message, args) {
        ...
        switch(args[0]) {
            ...
            case 'rm':
                ...
                console.log(t);
                if (!args[1]) return message.channel.send(`You need to specify the reaction role ID. Check \`${index.prefix}rr list\` to see the IDs.`);
                if (!t) return message.channel.send(`I can\'t find that reaction role. Check \`${index.prefix}rr list\` to see the reaction role IDs.`);
                let rguild = client.guilds.get(t.guildID);
                if (!rguild) return message.channel.send(`I can\'t find that reaction role. Check \`${index.prefix}rr list\` to see the reaction role IDs.`);
                if (rguild.id != message.guild.id) return message.channel.send(`I can\'t find that reaction role. Check \`${index.prefix}rr list\` to see the reaction role IDs.`);
            ...
            default: 
                ...
                .setDescription(`Let your members give roles to themselves by reacting to a message.\n\n\`${index.prefix}rr create → Create a new reaction role.\`\n\n\`${index.prefix}rr list → List your reaction roles.\`\n\n\`${index.prefix}rr delete <ID> → Delete a reaction role.\nYou can get the ID with ${index.prefix}rr list.\``);
